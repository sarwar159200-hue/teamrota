...
import { amendLeaveDecision, assignBulkPastLeave, assignPastLeave, decideLeave, deleteLeaveHistory, submitLeave } from "./actions";import {canManageWorkforce} from "@/lib/access-control";
...
export default async function LeavePage({
  ...
  const historical = params.historical === "1";
  const bulkHistoricalRaw = Array.isArray(params.bulkHistorical) ? params.bulkHistorical[0] : params.bulkHistorical;
  const bulkHistoricalCount = Number(bulkHistoricalRaw || 0);
  ...
  return (
    <main className="standalone-page phase3-page">
      ...
      {historical && (
        <div className="form-feedback form-feedback-success" role="status">
          <strong>Historical leave recorded</strong>
          <span>The approved leave history, balance, rota and timesheet were updated.</span>
        </div>
      )}

      {bulkHistoricalCount > 0 && (
        <div className="form-feedback form-feedback-success" role="status">
          <strong>Bulk historical leave uploaded</strong>
          <span>{bulkHistoricalCount} separate historical leave date(s) were added successfully.</span>
        </div>
      )}
      ...
      {privileged && (
        <section className="panel admin-block bulk-history-panel">
          <div className="section-heading">
            <div>
              <Palmtree />
              <div>
                <h2>Bulk Historical Leave</h2>
                <p>Add many separate past leave dates for one employee in a single upload. Each date is saved as its own approved leave record.</p>
              </div>
            </div>
          </div>
          <form action={assignBulkPastLeave} className="form-grid" encType="multipart/form-data">
            <select name="employee_id" required defaultValue="">
              <option value="">Employee *</option>
              {(managedEmployees || []).map((employee: any) => (
                <option key={employee.id} value={employee.id}>
            ...
            <textarea
              ...
              placeholder={`Enter separate dates, one per line or separated by commas.
Examples:
05-May-2026
01-Jun-2026
15-Jun-2026
02-Jul-2026`}
              ...
              required
            />
            <textarea name="reason" placeholder="Reason / administrative note for this bulk history" required />
            <label className="file-input">
              <Paperclip size={17} /> Shared supporting document (optional unless required)
              <input name="medical_document" type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" />
            </label>
            <button className="primary-button" type="submit">Upload separate historical dates</button>
          </form>
          <small className="muted">Accepted date formats: YYYY-MM-DD, DD-MMM-YYYY, or DD/MM/YYYY. Maximum 100 dates per upload.</small>
        </section>
      )}
